# -------------------------------------------------
# Device, before import library using JAX
# Global config , affects everything
# Put it VERY early, NOT after creating arrays. 
# Before import jax.numpy as jnp !!
# -------------------------------------------------
import jax
...
def simulation_user_driven_via_CPU(
    ...
    # ----------------------------
    monitor = None
    if monitor_memory and any(d.platform == "gpu" for d in jax.devices()):
    ...
    for precompute_run in range(1, precompute_times+1):
        ...
        elements_jax, nodes_jax, elements_np, nodes_np = prepare_mesh_jax(
            nx, ny, nz, L, W, H, tet, dtype
        )

        precomp = precompute_func(nodes_jax, elements_jax, lam, mu)
        ...
        dofs_all, dofs_fix = precalculate_dofs_func(
            nodes_jax, elements_jax, W, H, ny, nz
        )

        for v in precomp.values():
            jax.block_until_ready(v)
        jax.block_until_ready(dofs_all)
        jax.block_until_ready(dofs_fix)
    ...
    while not glfw.window_should_close(window):
        ...
        # ------ show result ----------
            ...
            glfw.set_window_title(window, f"{n_elem} elements, {load}, {framework}, {version}, ::::::: Force: {force_val}")
