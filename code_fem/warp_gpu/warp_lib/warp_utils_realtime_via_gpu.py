...
# (3) Copy (memcpy)    -> GPU -> GPU (CUDA interop)
...
def update_deformed_vbo(n_nodes, u_wp, num_surface_vertices, nodes_render_wp, surfaces_wp, vbo_gpu, size, render_params ):
    ...
    #note: if vbo_gpu.ptr fails, then wp.get_cuda_device().ptr(vbo_gpu)
    cuda_gl_interop.memcpy_to_vbo(ptr, vbo_gpu.ptr, size)
...
def simulation_user_driven_GPU_w_copy(
        nx, ny, nz, 
        L, W, H, tet,
        lam, mu,
    ...
    # print("====================================================================================")
    # print(f"Mesh: {(nx-1)*(ny-1)*(nz-1)*tet} elements, framework: {framework}, load:{load}, version: {version},  data transfer: GPU.")
    # print("====================================================================================")
    ...
    for precompute_run in range(1, precompute_times+1):
        ...
        nodes_render_wp = wp.array(nodes_np, dtype=wp.float32)
        ...
        #data_records["precompute_time"] = precompute_t
    ...
    while not glfw.window_should_close(window):
        ...
        if state_mode == WAIT_INPUT:
            ...
            glfw.set_window_title(window, f"{n_elem} elements, {load}, {framework}, {version}, GPU ::::::: Enter force: {input_buffer}")
        ...
        # =========================
            ...
            if needs_update:
                ...
                update_deformed_vbo(n_nodes, u_wp, num_surface_vertices, nodes_render_wp, surfaces_wp, vbo_gpu, size, render_params )
            ...
            glDrawArrays(GL_TRIANGLES, 0, num_surface_vertices)
            #glDrawArrays(GL_TRIANGLES, 0, num_surface_vertices)
            glfw.set_window_title(window, f"{n_elem} elements, {load}, {framework}, {version}, GPU::::::: Force: {force_val}, Iterations: {iters}")
